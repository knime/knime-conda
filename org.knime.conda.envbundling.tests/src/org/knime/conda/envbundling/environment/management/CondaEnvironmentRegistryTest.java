...
public class CondaEnvironmentRegistryTest {
    ...
    public void testEnvironmentCreation() throws Exception {
        var environment = CondaEnvironmentRegistry.getOrCreateEnvironment(TEST_ENV);
        assertEquals(TEST_ENV, environment.getName());
        var configArea = Paths.get(Platform.getConfigurationLocation().getURL().toURI());
        var expectedPath = configArea.resolve("micromamba_envs").resolve(TEST_ENV);
